...
class Brand(Resource):
    ...
    def post(self):
        ...
        columns = ['first_name', 'last_name', 'email_id', 'company_name', 'password', 'role','default_currency']
        data = [first_name, last_name, email, company_name, password_sha, role,default_currency]
        result=0
        try:
...
class Brand(Resource):
    def get(self,user_id):
        ...
        columns = ['user_id', 'first_name', 'last_name', 'company_name', 'email_id', 'role','phone','position','url','country'
                   ...
                   'insta_url','youtube_url','profile_pic','confirmed_email','address_line','postal_code','tax_no','default_currency']
...
class Brand(Resource):
    def get(self,user_id,channel_name):
        '''get all  influencer alerts list by user_id and channel name'''
...
@ns_brand.route('/getInfluencerFavList_with_details_new/<string:user_id>/<string:channel_name>')
class Brand(Resource):
    def get(self,user_id,channel_name):
        ...
        if channel_name == 'youtube':
            columns = ['channel_id', 'title', 'channel_img', 'desc', 'subscriberCount_gained', 'subscriberCount_lost',
                       ...
                       'total_100video_views_unique', 'total_100video_likes', 'total_100video_dislikes',
                       'total_100video_comments', 'total_100video_shares',
                       'facebook_url', 'insta_url', 'twitter_url', 'country','channel_id','channel_name']
            connecsiObj = ConnecsiModel()
            data = connecsiObj.get_fav_inf_list_by_channel_name_new(user_id=user_id,channel_name=channel_name)
            response_list = []
            for item in data:
                dict_temp = dict(zip(columns, item))
                response_list.append(dict_temp)
...
class PlatFormAnalysis(Resource):
    def get(self,user_id):
        ...
        connecsiObj = ConnecsiModel()
        columns = ['proposal_id','campaign_id','inf_first_name','inf_last_name','inf_categories','inf_country','proposal_price','proposal_from_date','proposal_to_date','regions','video_cat_id','campaign_status','channel_id',
